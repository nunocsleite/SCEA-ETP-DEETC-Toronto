...
int Chromosome::getExamTimeslot(int exam) const {
    int period = 0;
    for (vector<unordered_set<int> >::const_iterator it = this->getConstPeriods().begin(); it != this->periods.end(); ++it) {
...
void Chromosome::toCSV(string const& filename) const {
    ...
    int period = 1;
    for (vector<unordered_set<int> >::const_iterator it = getConstPeriods().begin(); it != periods.end(); ++it) {
        for (unordered_set<int>::const_iterator examit = (*it).begin(); examit != (*it).end(); ++examit) {
