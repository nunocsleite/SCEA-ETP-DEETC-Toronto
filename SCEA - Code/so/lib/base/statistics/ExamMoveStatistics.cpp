...
#include "base/algorithms/statistics/moTAWithStatistics.h"
#include "ETTPInit.h"
...
ExamMoveStatistics::ExamMoveStatistics(TestSet const& _testSet, string const& _outputDir,
    ...
    : testSet(_testSet),                                  // Test set
      ...
      examInfoVector(testSet.getNumExams()),              // ExamInfo vector
      numBins(_numBins),
      ...
      outFile(outFilename)
{
    ...
    for (int examIndex = 0; examIndex < testSet.getNumExams(); ++examIndex) {
        examInfoVector[examIndex] =
///TODO
/// TORONTO EXAMS INDEXES START AT 1
                boost::make_shared<ExamInfo>(examIndex+1,        // int _examIndex
...
void ExamMoveStatistics::determineExamsColorDegree() {
    // Get exam graph
    AdjacencyList const& graph = *getInitialSolution().getGraph();
...
//        cout << "\n";
        ...
        // Set exam degree
        if (*vertexIt > 0)
            examInfoVector[*vertexIt-1]->setExamColorDegree(vertexDegree);
/// TODO
/// TORONTO EXAMS INDEXES START AT 1 ??
    }
}
...
void ExamMoveStatistics::run() {
    ...
    //
//    ETTPneighborhood<ETTPneighborProximityCost<eoChromosome> > neighborhood;
//    ETTPneighborEval<ETTPneighborProximityCost<eoChromosome> > neighEval;
//    ProximityCostEval<eoChromosome> fullEval;
    ...
    ProximityCostEval<eoChromosome> fullEval; // eoEvalFunc used to evaluate the solutions
    ETTPneighborhood<eoChromosome> neighborhood(kempeChainHeuristic, fullEval);
    ETTPneighborEval<ETTPneighborProximityCost<eoChromosome> > neighEval;


    // TA cooling schedule
//    moSimpleCoolingSchedule<eoChromosome> coolSchedule(0.1, 0.99, 3, 2e-5);
//        moSimpleCoolingSchedule<eoChromosome> coolSchedule(0.1, 0.001, 5, 2e-5); /// LAST
...
//    moTA<ETTPneighborProximityCost<eoChromosome> > ta(neighborhood, fullEval, neighEval, coolSchedule);

//    moTA<ETTPneighborProximityCost<eoChromosome> > ta(neighborhood, fullEval, neighEval, coolSchedule);

//    moTAWithStatistics<ETTPneighborProximityCost<eoChromosome> > ta(*this, neighborhood, fullEval, neighEval, coolSchedule);
    ...
    outFile << optimizedSolution.getChromosome() << endl;
    outFile << "==============================================================" << endl;
...
void ExamMoveStatistics::updateExamMove(int _exam, double _threshold) {
    ...
    examInfoVector[_exam-1]->updateMove(threshIndex);

/// TODO: TORONTO EXAMS INDEXES START AT 1?
///
}
